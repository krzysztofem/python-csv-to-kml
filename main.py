...
# https://www.mkrgeo-blog.com/easy-way-to-create-the-kml-files-with-python/

import csv
from datetime import datetime
import simplekml
import _config
from modules.getDescription import getDescription

kml = simplekml.Kml()

file = open('_source-for-map/znaleziska.csv', 'r')
...
DOC_PERIOD_1 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_1)
DOC_PERIOD_2 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_2)
DOC_PERIOD_3 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_3)
DOC_PERIOD_4 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_4)
DOC_PERIOD_5 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_5)
DOC_PERIOD_6 = kml.newdocument(name='Znaleziska %s w.' % PERIOD_6)
...
def createNewpoint(period):
    if period == PERIOD_1:
        return DOC_PERIOD_1.newpoint()
    elif period == PERIOD_2:
        return DOC_PERIOD_2.newpoint()
    elif period == PERIOD_3:
        return DOC_PERIOD_3.newpoint()
    elif period == PERIOD_4:
        return DOC_PERIOD_4.newpoint()
    elif period == PERIOD_5:
        return DOC_PERIOD_5.newpoint()
    else: 
        return DOC_PERIOD_6.newpoint()
